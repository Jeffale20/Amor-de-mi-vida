...
window.changeSong = function (song) {
    var player = document.getElementById('player');
    var source = document.getElementById('audioSource');

    if (player && source) {
        console.log("Cambiando a la canción:", song);
        source.src = song; // Cambia la fuente del archivo de audio
        player.load(); // Recarga el audio con la nueva canción
        player.play().catch(error => console.error("Error al reproducir la canción:", error));
    } else {
        console.error("No se encontraron los elementos del reproductor.");
    }
};
...
// Agregar el event listener para el botón de "Sí" en la propuesta de San Valentín
document.addEventListener("DOMContentLoaded", function () {
    document.getElementById("btnSi").addEventListener("click", mostrarRespuesta);
});