...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function loadLetters() {
        onSnapshot(collection(db, "cartas"), (snapshot) => {
            cardsContainer.innerHTML = "";
            snapshot.forEach((doc) => {
                addCardToDOM(doc.id, doc.data().text);
            });
        });
    }
    ...
    async function saveLetter() {
        ...
        if (letter !== "") {
            await addDoc(collection(db, "cartas"), { text: letter });
            showMessage("Carta guardada 💖");
            loveLetter.value = "";
        } else {
            showMessage("Escribe algo en la carta.");
        }
    }
    ...
    function addCardToDOM(id, text) {
        const cardElement = document.createElement("div");
        cardElement.classList.add("saved-card");
        cardElement.innerHTML = `<p>${text}</p><button class="delete-card">❌</button>`;
